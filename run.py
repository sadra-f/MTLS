...
def main():
    for i in range(1,6,1):
        ...
        for j in range(1,6,1):
            ...
            print(f'init : L{N_TIMELINES}D{DATASET_NUMBER}', datetime.datetime.now())
            ...
            if DO_SAVE_RESULTS:
                ...
                np.save(f'../results/alignm1/L{N_TIMELINES}D{DATASET_NUMBER}', np.array(evaluations_alignm1), allow_pickle=True)

                # with open(f'../Results/dselect/L{N_TIMELINES}D{DATASET_NUMBER}.txt', 'w') as f:
                np.save(f'../results/dselect/L{N_TIMELINES}D{DATASET_NUMBER}', np.array(evaluations_d_select), allow_pickle=True)

                # with open(f'../Result_timelines/L{N_TIMELINES}D{DATASET_NUMBER}.txt', 'w') as f:
                np.save(f'../Result_timelines/L{N_TIMELINES}D{DATASET_NUMBER}', np.array(timelines_clusters_sentences), allow_pickle=True)
    return

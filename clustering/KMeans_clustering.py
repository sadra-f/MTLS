...
#         for i, data in enumerate(self.data):
#             for j, centroid in enumerate(self.centroids):
#                 self._current_distances[i, j] = self._dist_func(data.vector,data.examplar_sen,centroid[0].vector,centroid[0].examplar_sen)                
#             clust_indx = np.argmin(self._current_distances.take(i, 0))
#             self.labels[i] = clust_indx
#             self.centroids[clust_indx][1] += 1
...
#         self._set_to_sums()
#         for value in self.centroids:
#             value[0].vector = np.divide(value[0], value[1])
#             value[1] = 0

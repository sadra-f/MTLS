...
class DateParser: #the specificity goes as high as days
    ...
    FULL_TIME_PTRN = "^\d{4}-\d{2}-\d{2}T[a-zA-Z0-9:]+$" # 2008-09-16\\1743.htm.txt, 0, TIME, 2008-09-16T16:30 | 2008-09-15\\103.htm.txt, 10, TIME, 2023-05-11TMO
    ...
    YEAR_WEEK_PTRN = "^\d{4}-W\d+$" #2008-09-16\\1743.htm.txt, 17, DATE, 2023-W18
    YEAR_WEEK_WEEKEND_PTRN = "^\d{4}-W\d+-WE$" #2008-09-15\\103.htm.txt, 0, DATE, 2023-W19-WE

    DECADE_PTRN = "^\d{3}(X|x)?$" #2008-09-15\\103.htm.txt, 25, DATE, 201 | 2008-09-12\\67.htm.txt, 6, DATE, 201X
    ...
    def parse(date_str:str, default_date:date = None, do_log=False) -> date:
        ...
        try:
            if re.search(DateParser.FULL_DATE_PTRN, date_str) is not None:
                ...
                values = date_str.split('-')
                res = date(int(values[0]), int(int(values[1][1:]) / 4) + 1, DateParser.DEFAULT_DAY)
